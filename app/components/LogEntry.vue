...
<template>
  <div
    :class="['log-entry', `level-${entry.level}`, { 'cursor-pointer': entry.isJson || entry.metadata }]"
    @click="entry.isJson || entry.metadata ? emit('toggle') : emit('select')"
  >
    <div class="flex items-start gap-3">
      <!-- Line number -->
      <span class="text-[var(--log-text-muted)] font-mono text-xs w-12 text-right shrink-0 select-none">
        {{ entry.lineNumber }}
      </span>
      ...
      >
        <UIcon 
          :name="entry.expanded ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'" 
    ...
    >
      <pre class="text-sm font-mono text-[var(--log-text-muted)] whitespace-pre-wrap"><code>{{ formatJson(entry.metadata) }}</code></pre>
    </div>
  </div>
</template>
